function model = plotBehaviorDependence(modelFile)
  ...
  [pan,shape,fig]     = makePanels([2,numel(model)], '', '', 'aspectratio', 1.8, 'maxsubplotsize', 600, 'panelmargins', struct('b',40));
  ...
  for iAni = 1:numel(aniModel)
    ...
    errorbar(axs, varIndex - 0.05*(2*iAni-3), fitW, -ciW(:,1), ciW(:,2), 'linestyle', 'none', 'color', aniColor(iAni,:), 'marker', 'o', 'markersize', 5, 'linewidth', 1.5);
  end
  ...
  for iModel = 1:numel(model)
    ...
    %% Axes and labels 
    axs               = selectPanel(pan, [2,iModel], shape);
    hold(axs, 'on');
